...
void	sort3_position_4(t_list *stack)
{
	...
	remain = 3 - max_rotate;
	while (max_rotate--)
		do_rrb(stack); //finding max
	do_pa(stack); //push max
	while (remain--)
		do_rrb(stack); //move remaining number form botom
	do_pa(stack); //move 2 remaining number to a
	do_pa(stack); //move 2 remaining number to a
	compare_swap(stack, 1); //swap if it needed
}
